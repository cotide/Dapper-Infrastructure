...
namespace BH.Framework.Utility
{
    ...
    public class Captcha
    {
        ...
        /// 创建验证码的图片
        /// </summary>
        /// <param name="captchaObject">要输出到的page对象</param>
        /// <returns>二进制图片</returns>
        public byte[] CreateValidateGraphic(string captchaObject)
        {
            var image = new Bitmap((int)Math.Ceiling(captchaObject.Length * 12.0), 22);
            ...
            try
            {
                ...
                g.DrawString(captchaObject, font, brush, 3, 2);
                //画图片的前景干扰点
                for (var i = 0; i < 100; i++)
                {
