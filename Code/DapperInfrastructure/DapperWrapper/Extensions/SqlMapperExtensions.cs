...
using Dapper;
using DapperInfrastructure.DapperWrapper.Core;
...
namespace Dapper.Contrib.Extensions.Option
{
    /// <summary>
    ...
    public static partial class SqlMapperExtensions
    {
        ...
        private static readonly ConcurrentDictionary<RuntimeTypeHandle, IDictionary<string, ColumnPropertyInfo>> TypeColumProperties = new ConcurrentDictionary<RuntimeTypeHandle, IDictionary<string, ColumnPropertyInfo>>();
        ...
        private static List<PropertyInfo> TypePropertiesCache(Type type)
        {
            ...
            // Colum属性
            var columProperties = properties.ToDictionary(item => item.Name, item => new ColumnPropertyInfo(item));
            TypeColumProperties[type.TypeHandle] = columProperties;
        ...
        public static long Insert<T>(this IDbConnection connection, T entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            for (var i = 0; i < allPropertiesExceptKeyAndComputed.Count; i++)
            {
                ...
                var columName = property.Name;

                if (type != null && TypeColumProperties.ContainsKey(type.TypeHandle))
                {
                    if (TypeColumProperties[type.TypeHandle].ContainsKey(property.Name))
                    {
                        columName = TypeColumProperties[type.TypeHandle][property.Name].ColumnName;
                    }
                }

                adapter.AppendColumnName(sbColumnList, columName);  //fix for issue #336
        ...
        public static bool Update<T>(this IDbConnection connection, T entityToUpdate, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            for (var i = 0; i < nonIdProps.Count; i++)
            {
                ...
                adapter.AppendColumnNameEqualsValue(sb, columName);  //fix for issue #336 
                parm.Add(columName, GetParamsValue(entityToUpdate,property.Name));
            ...
            var updated = connection.Execute(sb.ToString(), parm, commandTimeout: commandTimeout, transaction: transaction);
            return updated > 0;
        }


        private static object GetParamsValue(object obj,string name)
        {
...
public partial class MySqlAdapter : ISqlAdapter
{
    ...
    /// <param name="sb">The string builder  to append to.</param>
    /// <param name="columnName">The column name.</param>
    /// <param name="columnValueKey"></param>
    public void AppendColumnNameEqualsValue(StringBuilder sb, string columnName)
    {
        sb.AppendFormat("`{0}` = @{1}", columnName, columnName);
    }
